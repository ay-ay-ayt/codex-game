...
function prepareCockpitOverlay(frameImg, overlayEl, glassEl) {
  ...
  const applyMask = () => {
    ...
    if (glassEl) {
      ...
      if (glassCtx) {
        const glassData = glassCtx.createImageData(width, height);
        const glassPixels = glassData.data;
        for (let i = 0, px = 0; i < glassPixels.length; i += 4, px += 1) {
          if (!expandedMask[px]) continue;
          glassPixels[i] = 175;
          glassPixels[i + 1] = 212;
          glassPixels[i + 2] = 238;
          glassPixels[i + 3] = 54;
        }
        glassCtx.putImageData(glassData, 0, 0);

        const highlight = glassCtx.createLinearGradient(0, 0, 0, height);
        highlight.addColorStop(0, "rgba(224, 242, 255, 0.35)");
        highlight.addColorStop(0.5, "rgba(184, 220, 248, 0.15)");
        highlight.addColorStop(1, "rgba(120, 160, 196, 0.2)");
        glassCtx.globalCompositeOperation = "source-atop";
        glassCtx.fillStyle = highlight;
        glassCtx.fillRect(0, 0, width, height);
        glassEl.style.backgroundImage = `url(${glassCanvas.toDataURL("image/png")})`;
      }
    }
