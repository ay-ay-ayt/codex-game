...
function createFighter(color, isPlayer = false) {
  ...
  // triangular-like main wing with a longer, nearly parallel rear edge
  ...
  const wingCenter = new THREE.Mesh(new THREE.BoxGeometry(16.8, 1.1, 15.4), bodyMat);
  wingCenter.position.set(-2.6, -1.7, 0);
  ...
  // triangular-like tailplanes with a near-parallel rear edge; moved aft to open gap from main wing
  ...
  subWingL.position.set(-10.0, 1.7, 0);
  ...
  burnerL.position.set(-27.0, 1.9, 3.35);
  ...
  heatRingL.position.set(-26.1, 1.9, 3.35);
  const heatRingR = heatRingL.clone();
  heatRingR.position.z = -3.35;

  // twin vertical fins: larger and much further aft, with forward-leaning triangle/trapezoid direction
  const finShape = [
    [-7.0, 0.0],
    [0.2, 0.0],
    [1.2, 9.4],
  ...
  const finL = new THREE.Mesh(buildVerticalSurface(finShape, 0.58), bodyMat);
  finL.position.set(-28.6, 2.1, 5.85);
  finL.rotation.set(0.0, 0.12, 0.03);
  const finR = finL.clone();
  finR.position.z = -5.85;
  finR.rotation.set(0.0, -0.12, -0.03);

  const finBaseL = new THREE.Mesh(new THREE.BoxGeometry(6.8, 1.34, 3.0), bodyMat);
  finBaseL.position.set(-26.0, 1.96, 5.65);
  finBaseL.rotation.y = 0.11;
  const finBaseR = finBaseL.clone();
  finBaseR.position.z = -5.65;
