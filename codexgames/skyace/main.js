...
function createFighter(color, isPlayer = false) {
  ...
  const mirrorPoints = (points) => points.map(([x, z]) => [x, -z]);
  ...
  const flameOuterMat = new THREE.MeshBasicMaterial({
    color: isPlayer ? 0x55cfff : 0xffa66d,
    transparent: true,
    opacity: 0.72,
  });
  const flameInnerMat = new THREE.MeshBasicMaterial({
    color: isPlayer ? 0xb5f2ff : 0xffddba,
    transparent: true,
    opacity: 0.85,
  });

  const flameOuterL = new THREE.Mesh(new THREE.ConeGeometry(0.58, 4.4, 16), flameOuterMat);
  flameOuterL.rotation.z = Math.PI * 0.5;
  flameOuterL.position.set(-28.4, 2.0, 3.15);
  const flameOuterR = flameOuterL.clone();
  flameOuterR.position.z = -3.15;

  const flameInnerL = new THREE.Mesh(new THREE.ConeGeometry(0.32, 3.0, 14), flameInnerMat);
  flameInnerL.rotation.z = Math.PI * 0.5;
  flameInnerL.position.set(-27.6, 2.0, 3.15);
  ...
  const glow = new THREE.Mesh(
    new THREE.SphereGeometry(isPlayer ? 1.4 : 1.6, 12, 10),
    new THREE.MeshBasicMaterial({ color: isPlayer ? 0x67eaff : 0xff9b5a, transparent: true, opacity: 0.75 })
  );
  glow.position.set(-25.9, 1.35, 0);
...
function updatePlaneExhaust(plane, boostLevel = 0) {
  ...
  const wobbleA = 0.94 + Math.sin(t + plane.mesh.id * 0.37) * 0.08;
  const wobbleB = 0.95 + Math.cos(t * 1.13 + plane.mesh.id * 0.21) * 0.09;
  const strength = 1 + boostLevel * 1.7;

  plane.exhaust.outerFlames.forEach((flame, i) => {
    flame.scale.set(strength * (0.95 + i * 0.04), wobbleA * strength, wobbleB);
    flame.material.opacity = clamp(0.62 + boostLevel * 0.28, 0.4, 0.98);
  });
  plane.exhaust.innerFlames.forEach((flame, i) => {
    flame.scale.set(strength * (1.05 + i * 0.03), wobbleB * strength, wobbleA);
    flame.material.opacity = clamp(0.72 + boostLevel * 0.24, 0.45, 0.99);
  });
  plane.exhaust.burners.forEach((burner) => {
    burner.material.emissiveIntensity = 0.5 + boostLevel * 1.4;
  });
  const glow = plane.exhaust.glow;
  glow.scale.setScalar(1 + boostLevel * 0.9);
  glow.material.opacity = clamp(0.65 + boostLevel * 0.3, 0.45, 0.98);
}
