...
function updatePlaneExhaust(plane, boostLevel = 0) {
  ...
  const coreOpacityIdle = clamp(0.24 + boostLevel * 0.28 + pulse * 0.03, 0.14, 0.62);
  const coreOpacityBoost = clamp(0.58 + boostLevel * 0.4 + pulse * 0.05, 0.42, 1.0);
  ...
  const innerBlueDepth = clamp(0.86 + boostMix * 0.14 + pulse * 0.02, 0.84, 1.0);
  const innerWarm = clamp(0.44 + boostMix * 0.56 + pulse * 0.03, 0.42, 1.0);
  const innerGreen = clamp(0.54 + boostMix * 0.48 + pulse * 0.03, 0.52, 1.0);
  ...
  plane.exhaust.nozzleHeatLines.children.forEach((streak, index) => {
    ...
    streak.material.opacity = clamp(0.42 + streakPulse * 0.34 + boostMix * 0.08, 0.42, 0.86);
