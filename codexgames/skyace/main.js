...
function updatePlaneExhaust(plane, boostLevel = 0) {
  ...
  const coreOpacityBoost = clamp(0.68 + boostLevel * 0.46 + pulse * 0.05, 0.52, 1.0);
  ...
  const innerBlueDepth = clamp(0.82 + boostMix * 0.18 + pulse * 0.02, 0.8, 1.0);
  const innerWarm = clamp(0.34 + boostMix * 0.66 + pulse * 0.03, 0.32, 1.0);
  const innerGreen = clamp(0.44 + boostMix * 0.56 + pulse * 0.03, 0.42, 1.0);
