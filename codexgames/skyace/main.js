...
function createFighter(color, isPlayer = false) {
  ...
  const burnerCoreMat = new THREE.MeshStandardMaterial({
    color: isPlayer ? 0x9de7ff : 0xffbe8e,
    emissive: isPlayer ? 0x7edbff : 0xff9650,
    emissiveIntensity: isPlayer ? 0.9 : 0.72,
    roughness: 0.16,
    metalness: 0.62,
  });
  ...
  const flameOuterMat = new THREE.MeshBasicMaterial({
    color: isPlayer ? 0x6fd6ff : 0xffb170,
    transparent: true,
    opacity: 0.84,
  ...
  const flameOuterL = new THREE.Mesh(new THREE.ConeGeometry(1.18, 4.8, 22), flameOuterMat);
  flameOuterL.rotation.z = Math.PI * 0.5;
  flameOuterL.position.set(-28.7, 2.0, 3.15);
  const flameOuterR = flameOuterL.clone();
  flameOuterR.position.z = -3.15;

  const nozzleHeatMat = new THREE.MeshBasicMaterial({
    color: isPlayer ? 0xffab5a : 0xff8c38,
    transparent: true,
    opacity: 0.72,
    blending: THREE.AdditiveBlending,
    depthWrite: false,
  });
  const nozzleHeatL = new THREE.Mesh(new THREE.TorusGeometry(0.82, 0.18, 10, 20), nozzleHeatMat);
  nozzleHeatL.rotation.y = Math.PI * 0.5;
  nozzleHeatL.position.set(-26.0, 2.0, 3.15);
  const nozzleHeatR = nozzleHeatL.clone();
  nozzleHeatR.position.z = -3.15;
  ...
  const plane = {
    ...
    exhaust: {
      ...
      nozzleHeat: [nozzleHeatL, nozzleHeatR],
      glows: [glowL, glowR],
    },
  };
...
function updatePlaneExhaust(plane, boostLevel = 0) {
  ...
  const pulseA = 0.92 + Math.sin(t + plane.mesh.id * 0.31) * 0.1;
  const pulseB = 0.96 + Math.cos(t * 1.15 + plane.mesh.id * 0.2) * 0.08;
  const widthGain = 1 + boostLevel * 0.55;
  const lengthGain = 1 + boostLevel * 1.28;

  plane.exhaust.outerFlames.forEach((flame, i) => {
    flame.scale.set(
      pulseA * widthGain * (0.98 + i * 0.02),
      pulseB * lengthGain * (1.05 + boostLevel * 0.25),
      pulseA * widthGain
    );
    flame.material.opacity = clamp(0.8 + boostLevel * 0.16, 0.5, 0.99);
  });

  plane.exhaust.burners.forEach((burner) => {
    burner.material.emissiveIntensity = 0.82 + boostLevel * 1.45;
  });

  plane.exhaust.nozzleHeat?.forEach((ring) => {
    ring.scale.set(1 + boostLevel * 0.14, 1 + boostLevel * 0.14, 1 + boostLevel * 0.14);
    ring.material.opacity = clamp(0.62 + boostLevel * 0.2, 0.35, 0.95);
  });

  plane.exhaust.glows?.forEach((glow) => {
    glow.scale.set(1 + boostLevel * 0.45, 1 + boostLevel * 0.95, 1 + boostLevel * 0.45);
    glow.material.opacity = clamp(0.68 + boostLevel * 0.24, 0.4, 0.99);
  });
}
