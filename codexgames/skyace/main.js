...
function updatePlaneExhaust(plane, boostLevel = 0) {
  ...
  plane.exhaust.redWisps.forEach((wisp) => {
    ...
    const boostOnly = THREE.MathUtils.smoothstep(boostMix, 0.38, 0.98);
    const wispMix = clamp((0.45 + boostMix * 1.75) * boostOnly, 0, 2.2);
    const wispLen = 1 + (0.9 + wispMix * 0.84 + burst * 0.32 - 1) * boostOnly;
    const wispRad = 1 + (0.86 + wispMix * 0.34 - 1) * boostOnly;
    wisp.scale.set(wispRad, wispLen, wispRad);
    const baseX = wisp.userData.baseX ?? wisp.position.x;
    wisp.position.x = baseX - (wispLen - 1) * 2.1;
    wisp.position.z = (wisp.userData.baseZ ?? wisp.position.z) + Math.sin(phase * 1.4) * 0.05 * boostOnly;
    const boostRedGain = (1.2 + boostMix * 2.1) * boostOnly;
    wisp.material.opacity = clamp((0.065 + burst * 0.2) * wispMix * boostRedGain, 0, 0.9);
