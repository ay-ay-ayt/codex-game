...
function createFighter(color, isPlayer = false) {
  ...
  const tailL = new THREE.Mesh(new THREE.BoxGeometry(12.0, 0.22, 3.0), wingMat);
  tailL.position.set(-17.5, 2.6, 6.0);
  tailL.rotation.y = 0.54;
  const tailR = tailL.clone();
  tailR.position.z = -6.0;
  tailR.rotation.y = -0.54;

  const finL = new THREE.Mesh(new THREE.BoxGeometry(0.26, 10.5, 1.2), bodyMat);
  finL.position.set(-13.9, 7.9, 2.8);
  finL.rotation.set(0.02, 0.24, -0.08);
  const finR = finL.clone();
  finR.position.z = -2.8;
  finR.rotation.set(-0.02, -0.24, 0.08);
