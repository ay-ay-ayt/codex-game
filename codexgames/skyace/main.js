...
function createFighter(color, isPlayer = false) {
  ...
  wingL.position.set(0.25, -1.42, 0);
  wingL.rotation.x = 0.018;
  ...
  const flameOuterL = new THREE.Mesh(new THREE.ConeGeometry(1.08, 5.4, 20), flameOuterMat);
  flameOuterL.rotation.z = Math.PI * 0.5;
  flameOuterL.position.set(-29.1, 2.0, 3.15);
  const flameOuterR = flameOuterL.clone();
  flameOuterR.position.z = -3.15;

  const flameInnerL = new THREE.Mesh(new THREE.ConeGeometry(0.7, 4.0, 18), flameInnerMat);
  flameInnerL.rotation.z = Math.PI * 0.5;
  flameInnerL.position.set(-28.2, 2.0, 3.15);
  ...
  const flameCoreL = new THREE.Mesh(new THREE.ConeGeometry(0.34, 1.7, 14), flameCoreMat);
  flameCoreL.rotation.z = Math.PI * 0.5;
  flameCoreL.position.set(-27.1, 2.0, 3.15);
  ...
  tailL.position.set(-0.1, 1.45, 0);
  tailL.rotation.x = 0.01;
  ...
  const glowL = new THREE.Mesh(new THREE.CylinderGeometry(0.82, 1.06, 1.9, 16), glowMat);
...
function updatePlaneExhaust(plane, boostLevel = 0) {
  ...
  plane.exhaust.innerFlames.forEach((flame, i) => {
    flame.scale.set(
      (0.9 + i * 0.04) * (1 + boostLevel * 0.35),
      pulseB * strength * (1.0 + boostLevel * 0.45),
      pulseA * (1 + boostLevel * 0.26)
    );
  ...
  plane.exhaust.cores?.forEach((core) => {
    core.scale.set(1 + boostLevel * 0.3, 1 + boostLevel * 0.55, 1 + boostLevel * 0.3);
    core.material.opacity = clamp(0.84 + boostLevel * 0.14, 0.5, 0.99);
  });

  plane.exhaust.glows?.forEach((glow) => {
    glow.scale.set(1 + boostLevel * 0.5, 1 + boostLevel * 0.6, 1 + boostLevel * 0.5);
