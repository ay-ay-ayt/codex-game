...
function createFighter(color, isPlayer = false) {
  ...
  const wingMat = new THREE.MeshStandardMaterial({
    color: isPlayer ? 0xe6edf3 : 0xe8d8bf,
  ...
  const fuselageProfile = [
    new THREE.Vector2(0.1, -31),
    new THREE.Vector2(0.35, -27),
    new THREE.Vector2(0.95, -20.5),
    new THREE.Vector2(1.8, -11.5),
    new THREE.Vector2(2.65, -2.2),
    new THREE.Vector2(2.9, 8.5),
    new THREE.Vector2(2.35, 17.2),
    new THREE.Vector2(1.4, 24.0),
    new THREE.Vector2(0.52, 29.5),
  ];
  ...
  const nose = new THREE.Mesh(new THREE.ConeGeometry(1.02, 7.8, 20), wingMat);
  nose.rotation.z = -Math.PI * 0.5;
  nose.position.set(34.2, 0, 0);

  const canopyDeck = new THREE.Mesh(new THREE.BoxGeometry(9.8, 1.2, 2.4), bodyMat);
  canopyDeck.position.set(12.6, 2.0, 0);
  const canopy = new THREE.Mesh(
    new THREE.CapsuleGeometry(1.85, 6.0, 7, 16),
    new THREE.MeshStandardMaterial({ color: 0xc6f2ff, transparent: true, opacity: 0.76, roughness: 0.06, metalness: 0.18 })
  );
  canopy.rotation.z = Math.PI * 0.5;
  canopy.scale.set(2.06, 1.0, 0.92);
  canopy.position.set(12.2, 3.15, 0);

  const dorsalSpine = new THREE.Mesh(new THREE.BoxGeometry(20.0, 1.0, 2.8), bodyMat);
  dorsalSpine.position.set(-0.8, 2.2, 0);

  // Main wing: broad trapezoid, reference-like
  const mainWingPlan = [
    [14.2, 2.8],
    [8.8, 11.6],
    [-6.8, 17.2],
  ...
  const mainWingL = new THREE.Mesh(buildSurface(mainWingPlan, 0.5), wingMat);
  mainWingL.position.set(0.2, -2.0, 0);
  mainWingL.rotation.x = -0.03;
  const mainWingR = new THREE.Mesh(buildSurface(mirrorPoints(mainWingPlan), 0.5), wingMat);
  mainWingR.position.copy(mainWingL.position);
  mainWingR.rotation.copy(mainWingL.rotation);

  const wingBridge = new THREE.Mesh(new THREE.BoxGeometry(18.8, 1.0, 14.5), bodyMat);
  wingBridge.position.set(-1.6, -1.72, 0);
  const wingBlend = new THREE.Mesh(new THREE.CapsuleGeometry(1.35, 8.6, 8, 16), bodyMat);
  wingBlend.rotation.z = Math.PI * 0.5;
  wingBlend.position.set(-2.2, -1.42, 0);

  // Shoulders / LERX
  const lERXL = new THREE.Mesh(buildSurface([
    [14.4, 1.1],
    [9.6, 6.7],
    [2.6, 5.6],
    [5.9, 1.8],
  ], 0.24), bodyMat);
  lERXL.position.set(0, -0.52, 0);
  const lERXR = new THREE.Mesh(buildSurface(mirrorPoints([
    [14.4, 1.1],
    [9.6, 6.7],
    [2.6, 5.6],
    [5.9, 1.8],
  ...
  // Twin engines
  const engineL = new THREE.Mesh(new THREE.CylinderGeometry(1.84, 2.18, 18.5, 20), bodyMat);
  engineL.rotation.z = -Math.PI * 0.5;
  engineL.position.set(-10.8, 2.05, 3.4);
  const engineR = engineL.clone();
  engineR.position.z = -3.4;

  const nozzleL = new THREE.Mesh(new THREE.CylinderGeometry(1.42, 1.94, 5.0, 20), darkMat);
  nozzleL.rotation.z = Math.PI * 0.5;
  nozzleL.position.set(-25.0, 1.95, 3.4);
  const nozzleR = nozzleL.clone();
  nozzleR.position.z = -3.4;

  const burnerMat = new THREE.MeshStandardMaterial({
    color: isPlayer ? 0x84ebff : 0xffb07d,
    emissive: isPlayer ? 0x5cdfff : 0xff8f4f,
    emissiveIntensity: 0.64,
    roughness: 0.14,
    metalness: 0.62,
  });
  const burnerL = new THREE.Mesh(new THREE.CylinderGeometry(0.9, 1.18, 2.0, 18), burnerMat);
  burnerL.rotation.z = Math.PI * 0.5;
  burnerL.position.set(-27.1, 1.95, 3.4);
  const burnerR = burnerL.clone();
  burnerR.position.z = -3.4;

  const flameMat = new THREE.MeshBasicMaterial({
    color: isPlayer ? 0x5dd8ff : 0xffa871,
    transparent: true,
    opacity: 0.8,
    blending: THREE.AdditiveBlending,
    depthWrite: false,
  });
  const flameL = new THREE.Mesh(new THREE.ConeGeometry(1.2, 5.3, 18), flameMat);
  flameL.rotation.z = Math.PI * 0.5;
  flameL.position.set(-29.8, 1.95, 3.4);
  const flameR = flameL.clone();
  flameR.position.z = -3.4;
  ...
  const heatRingL = new THREE.Mesh(new THREE.TorusGeometry(1.05, 0.18, 10, 20), heatRingMat);
  heatRingL.rotation.y = Math.PI * 0.5;
  heatRingL.position.set(-26.2, 1.95, 3.4);
  const heatRingR = heatRingL.clone();
  heatRingR.position.z = -3.4;

  // Aft sub wings (horizontal tails) clearly smaller and rear
  const subWingL = new THREE.Mesh(new THREE.BoxGeometry(8.4, 0.22, 2.4), wingMat);
  subWingL.position.set(-22.4, 0.7, 7.8);
  subWingL.rotation.y = 0.5;
  const subWingR = subWingL.clone();
  subWingR.position.z = -7.8;
  subWingR.rotation.y = -0.5;

  // Vertical fins: side of engines, behind main wing, around subwing x-zone
  const finL = new THREE.Mesh(new THREE.BoxGeometry(0.28, 8.4, 1.35), bodyMat);
  finL.position.set(-20.6, 4.8, 4.8);
  finL.rotation.set(0.02, 0.2, -0.06);
  const finR = finL.clone();
  finR.position.z = -4.8;
  finR.rotation.set(-0.02, -0.2, 0.06);

  const finBaseL = new THREE.Mesh(new THREE.BoxGeometry(3.8, 1.0, 1.6), bodyMat);
  finBaseL.position.set(-19.2, 2.25, 4.8);
  finBaseL.rotation.y = 0.18;
  const finBaseR = finBaseL.clone();
  finBaseR.position.z = -4.8;
  finBaseR.rotation.y = -0.18;

  const intakeL = new THREE.Mesh(new THREE.BoxGeometry(6.2, 1.7, 1.5), darkMat);
  intakeL.position.set(10.2, 0.28, 3.55);
  const intakeR = intakeL.clone();
  intakeR.position.z = -3.55;

  g.add(
    fuselage, nose, canopyDeck, canopy, dorsalSpine,
    wingBridge, wingBlend, mainWingL, mainWingR,
    lERXL, lERXR,
    engineL, engineR, nozzleL, nozzleR, burnerL, burnerR,
    flameL, flameR, heatRingL, heatRingR,
    subWingL, subWingR,
    finL, finR, finBaseL, finBaseR,
    intakeL, intakeR
  );
...
function updatePlaneExhaust(plane, boostLevel = 0) {
  ...
  const radiusGain = 1 + boostLevel * 0.72;
  const lengthGain = 1 + boostLevel * 1.25;
