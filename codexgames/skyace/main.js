...
function updatePlaneExhaust(plane, boostLevel = 0) {
  ...
  const coreOpacityIdle = clamp(0.32 + boostLevel * 0.34 + pulse * 0.04, 0.2, 0.76);
  const coreOpacityBoost = clamp(0.74 + boostLevel * 0.44 + pulse * 0.06, 0.56, 1.0);
  ...
  const innerBlueDepth = clamp(0.92 + boostMix * 0.08 + pulse * 0.025, 0.9, 1.0);
  const innerWarm = clamp(0.58 + boostMix * 0.5 + pulse * 0.04, 0.56, 1.0);
  const innerGreen = clamp(0.66 + boostMix * 0.4 + pulse * 0.04, 0.64, 1.0);
  ...
  plane.exhaust.nozzleHeatLines.children.forEach((streak, index) => {
    ...
    streak.material.opacity = clamp(0.52 + streakPulse * 0.4 + boostMix * 0.1, 0.52, 0.94);
